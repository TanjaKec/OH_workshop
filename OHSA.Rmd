...
It is alwas a good idea to have a look at data you upload, before you start using it for your analysis.
```{r}
# scan data
glimpse(mydata)
```

We can do with some tweaking, by removing a few variables and separating day from time and creating separate columns for latitude and longitude variables.
```{r}
# remoce 2nd and the last column
...
To see where the stations are allocated we will plot them on the Google maps. We will use the `leaflet` package to do this and create a sub data with only list of the stations and their positions.
...
summary(stations)
```
Once we have the subset data we can plot it using the `leaflet` package like below.
...
```
We can notice a few stations outside of Belgrade. By clicking on their location points on the map, we can identify them and remove them from our data set, as we want to focus only on Belgrade's stations.
...
## Analysisng the readings

In this section we will look through the readings and try to make some sense of it all.
If you are not much familiar with the information collected by the stations, ie. about the pm particles you can check the following link:  <https://www.irceline.be/en/documentation/faq/what-is-pm10-and-pm2.5>.

### No reading per week days

Next chunk of code counts the number of readings per each week day, it shows it in a table and visualises it using the `ggplot2()` package.
...
mydata_bg %>% 
  ...
  theme(legend.position="none") 
```
Those two plots look very similar. It would be interesting to see the bars of the two plots next to each other.
```{r}
mydata_new %>% 
  mutate(wday = wday(date, label = TRUE)) %>% 
  group_by(wday) %>% 
...
### Average concentration of the particles each month
...
### Average concentration of the particles each hour
```{r}
mydata_bg %>% 
  group_by(time) %>% 
  summarise(no_readings = n()) %>% 
...
In this section we will try to investigate if the concentration of pm particles depen upon other factors such as pressure and humidity.

### pm10 vs pressure

To investigate the relationship between the air pressure and concentration of pm10 paricles we will create a scatterplot:
...
The data is quite messy, with a number of very extreme readings, which is why we “chopped” the axis using the `xlim()` and the `ylim()` functions after checking the distributions of the two variables. This is a part of informal investigation of the possible relationship between `pm10` and `pressure`, and a more rigor procedure should be adopted which is out of the scope of this workshop.

### pm10 vs humidity

We will adopt the same approach to investigate possible relationship between the humidity and the concentration of the pm10 particles.
...
As a challenge try to use a scatterplot to investigate for a possible relationship between the concentration of the `p10` particles and the `temperature`.
